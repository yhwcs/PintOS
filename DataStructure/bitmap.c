...
struct bitmap* bitmap_expand(struct bitmap* b, size_t size){
	size_t b_size;
	b_size = bitmap_size(b);
	struct bitmap* new_bitmap;
	new_bitmap = bitmap_create(size+b_size);

	bool value;
	for(int i=0; i<b_size; i++){
		if(bitmap_test(b, i) == 1)
			value = true;
		else
			value = false;
		bitmap_set(new_bitmap, i, value);
	}
